...
FROM 'D:\titanic.csv' DELIMITER ',' CSV HEADER;
...
SELECT 
	passengerid 
,	CASE WHEN ROUND(age, 0) IS NULL 
		 THEN 30 ELSE ROUND(age, 0) 
		 END AS age 
FROM public.titanic; 
...
UPDATE public.titanic 
SET age = updated_values.age 
FROM updated_values 
WHERE titanic.passengerid = updated_values.passengerid;
...
SELECT 
	passengerid 
,	CASE WHEN cabin IS NULL 
		 THEN 'B96 B98' ELSE cabin 
		 END AS cabin 
FROM public.titanic; 
...
UPDATE public.titanic 
SET cabin = updated_values.cabin 
FROM updated_values 
WHERE titanic.passengerid = updated_values.passengerid;
...
-- ** populating the title column ** --
UPDATE public.titanic 
SET first_name = trim(split_part(split_part(name, ',', 2), '.', 2));
