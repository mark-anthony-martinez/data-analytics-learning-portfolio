...
FROM 'D:\property_sales.csv' DELIMITER ',' CSV HEADER;
...
SET lotfrontage = updated_values.lotfrontage 
FROM updated_values 
WHERE property_sales.id = updated_values.id;


-- ** converting lotfrontage to INTEGER ** --
ALTER TABLE public.property_sales 
ALTER COLUMN LotFrontage TYPE integer 
...
UPDATE public.property_sales 
SET gentle_slope = CASE WHEN landslope = 'Gentle slope' 
						THEN 1 ELSE 0 END;
...
UPDATE public.property_sales 
SET moderate_slope = CASE WHEN landslope = 'Moderate Slope' 
						  THEN 1 ELSE 0 END 
...
UPDATE public.property_sales 
SET severe_slope = CASE WHEN landslope = 'Severe Slope' 
					    THEN 1 ELSE 0 END
...
UPDATE public.property_sales 
SET OverallCond = updated_values.OverallCond 
FROM updated_values 
WHERE property_sales.id = updated_values.id;
...
,	LotArea 
,	ROUND(AVG(LotArea)
		OVER(PARTITION BY LotShape), 2)::VARCHAR AS mean_lotarea 
FROM public.property_sales
) 
UPDATE public.property_sales 
SET LotShape = updated_values.mean_lotarea 
FROM updated_values 
WHERE property_sales.id = updated_values.id;
...
-- ** converting to float ** --
ALTER TABLE public.property_sales 
ALTER COLUMN LotShape TYPE FLOAT 
USING LotShape :: FLOAT;
